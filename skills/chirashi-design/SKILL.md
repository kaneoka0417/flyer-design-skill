...
素材が不足している場合、または写真では表現しにくいビジュアル（抽象背景、カスタムイラスト、装飾要素）が必要な場合、nano-banana によるAI画像生成を提案できる。

**提案タイミング:**
- ユーザーが写真を持っていない場合
- 写真候補の評価がB/C以下で品質不足の場合
- 構成案の画像要件に対して候補が不足している場合

**必ずユーザーに確認してから生成する。** 詳細は `skills/nano-banana/SKILL.md` を参照。

**印刷品質の注意:**
- ヒーロー画像: 4K + pro モデル推奨（350dpi印刷対応）
- 特徴セクション: 1K-2K + flash で十分
- AI生成画像もStep 3.2のS/A/B/C評価を通すこと
